...
async def register_user(name: str, email: str, password: str) -> dict:
    ...
    if await email_exists(email) or await find_user_by_login(name):
...
async def login_user(login: str, password: str) -> dict:
    user = await find_user_by_login(login.strip())
...
async def get_current_user(token: str | None) -> dict:
    ...
    if await is_user_blocked(user["id"]):
        raise AuthError("blocked", "Аккаунт заблокирован")
    await touch_user_last_seen(user["id"])
    return user


async def logout_user(token: str | None) -> None:
    if token:
        await delete_session(token)


async def change_password(
    token: str | None, old_password: str, new_password: str
) -> None:
    user = await get_current_user(token)
    with_hash = await find_user_by_login(user["email"]) or {}
...
async def update_profile(token: str | None, name: str) -> dict:
    user = await get_current_user(token)
    name = name.strip()
    if not name or len(name) < 2 or len(name) > 50 or not _NAME_RE.match(name):
        raise AuthError("validation", "Некорректное имя")
    existing = await find_user_by_login(name)
    if existing and existing["id"] != user["id"]:
        raise AuthError("email_taken", "Пользователь уже зарегистрирован")
    updated = await set_user_name(user["id"], name)
    if not updated:
        raise AuthError("unauthorized", "Сессия истекла")
    return updated
...
async def seed_admin_user() -> None:
    ...
    """
    if await find_user_by_login("admin"):
        return
    await create_user("admin", "admin@animewatch.local", "admin", role="admin")
