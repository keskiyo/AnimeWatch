...
def acquire_refresh_lock(database_path: str) -> bool:
    """Take the refresh lock. Returns False when a live (non-expired) run holds it."""
    now = datetime.now(tz=UTC)
    current = get_sync_state(database_path, LOCK_KEY)
    if current:
        try:
    ...
    set_sync_state(database_path, LOCK_KEY, expires_at.isoformat())
    return True


def release_refresh_lock(database_path: str) -> None:
    set_sync_state(database_path, LOCK_KEY, "")


def is_refresh_running(database_path: str) -> bool:
    current = get_sync_state(database_path, LOCK_KEY)
    if not current:
        return False
    try:
...
def get_refresh_status(database_path: str) -> dict:
    summary_raw = get_sync_state(database_path, _STATE["last_summary"])
    try:
        last_summary = json.loads(summary_raw) if summary_raw else None
    except json.JSONDecodeError:
        last_summary = None
    return {
        "lastRunAt": get_sync_state(database_path, _STATE["last_run"]),
        "lastSuccessAt": get_sync_state(database_path, _STATE["last_success"]),
        "currentlyRunning": is_refresh_running(database_path),
        "lastStatus": get_sync_state(database_path, _STATE["last_status"]),
        "lastSummary": last_summary,
        "lastError": get_sync_state(database_path, _STATE["last_error"]) or None,
    }
...
async def run_catalog_refresh(settings: Settings | None = None) -> dict:
    ...
    if not acquire_refresh_lock(db):
        log.info("catalog_refresh_skipped_already_running")
        return {"status": "skipped_already_running"}

    started_at = datetime.now(tz=UTC).isoformat()
    set_sync_state(db, _STATE["last_run"], started_at)
    set_sync_state(db, _STATE["last_status"], "running")
    log.info("catalog_refresh_started started_at=%s", started_at)

    count_before = get_anime_catalog_stats(db)["count"]
    ...
    try:
        ...
        count_after = get_anime_catalog_stats(db)["count"]
        ...
        set_sync_state(db, _STATE["last_error"], str(exc)[:500])
        log.error("catalog_refresh_failed: %s", exc)
    finally:
        summary["finishedAt"] = datetime.now(tz=UTC).isoformat()
        set_sync_state(db, _STATE["last_status"], summary.get("status", "failed"))
        set_sync_state(db, _STATE["last_summary"], json.dumps(summary))
        release_refresh_lock(db)
