...
async def refresh_kodik_availability(settings: Settings | None = None) -> dict:
    ...
    try:
        ...
        set_sync_state(db, "kodik_availability_status", "failed")
        set_sync_state(db, "kodik_availability_last_error", str(exc))
    ...
    if not kodik_ids:
        ...
        set_sync_state(db, "kodik_availability_status", "empty")
        return {"status": "empty"}

    counts = mark_kodik_availability(db, kodik_ids)
    set_sync_state(db, "kodik_availability_status", "completed")
    set_sync_state(db, "kodik_availability_last_run_at", datetime.now(tz=UTC).isoformat())
    set_sync_state(
        db, "kodik_availability_counts", f"{counts['with']}/{counts['without']}"
    )
    log.info(
