...
- В каталоге/поиске скрываются только вышедшие и ongoing-тайтлы, где Kodik уже
  проверен и `has_kodik = 0`; анонсы остаются видимыми
- Сортировка по новизне (`aired_on`/дата старта), рейтингу и дате добавления,
...
Правило фильтра: `has_kodik = 0` скрывает только ongoing/released; `announced`
видны всегда, потому что у них ещё не может быть озвучки.
