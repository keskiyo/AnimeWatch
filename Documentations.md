...
| Слой             | Технологии                                                                   |
| ---------------- | ---------------------------------------------------------------------------- |
| Backend          | Python 3.12+, FastAPI (async), Uvicorn, httpx, MongoDB (Motor)               |
...
│   ├── data/avatars/           # загруженные аватары (создаётся сам)
│   └── src/
...
│       │   ├── static_pages.py #   публичные статические страницы из Mongo
...
│       ├── db/
│       │   ├── mongo.py         # Motor-клиент + get_db() (async)
│       │   ├── indexes.py       # ensure_indexes(): индексы коллекций + TTL
│       │   ├── anime_catalog.py / _queries.py / _lookup.py  # каталог: upsert/чтение
│       │   ├── sync_state.py / cache.py     # состояние sync + TTL-кеш (Mongo)
│       │   ├── static_pages.py              # коллекция статических страниц (CMS)
...
```
Shikimori ──(sync, вручную/еженедельно)──▶ Mongo `anime` ──▶ /api/anime/bulk ──▶ фронт
```

1. **Постоянная коллекция `anime`** в MongoDB (`_id` = shikimori id, int) —
   источник правды каталога. Переживает рестарты. Документы только upsert'ятся,
   никогда не удаляются.
2. **`/api/anime/bulk`** читает ТОЛЬКО из этой коллекции — Shikimori при обычной
   работе сайта не дёргается. Пустая коллекция → `{"data": [], "needs_sync": true}`.
3. **Фронт** (`useAnimeCache`) показывает сохранённый bulk сразу, затем один раз
   за JS-сессию перепроверяет `/api/anime/bulk`; фильтры, сортировка и поиск
   работают на клиенте мгновенно.
4. **Фильтр по озвучке Kodik.** В каталоге/поиске/рейле/студиях/sitemap
   скрываются только тайтлы, где Kodik уже проверен и озвучки нет. Это поле `has_kodik` в
   коллекции `anime` (отсутствует/`null` = не проверено → видно; `1` = есть →
   видно; `0` = проверено, нет → скрыто). Прячется только `has_kodik = 0`. **Исключение:**
   тайтлы со статусом `announced` (анонс) видны всегда — озвучки у них ещё нет,
   появится после выхода. Прямая ссылка
...
- **`useAnimeCache`** — zustand-стор каталога: сохранённый bulk из localStorage
  (`aw:catalog:v4`) показывается сразу, затем в фоне один раз за JS-сессию
  перепроверяется `/api/anime/bulk`. Общий клиентский стейт — на zustand
  (`useAuthUser`, `useAnimeCache`); локальный — `useState`. Redux не используется.
- **`useAnimeCatalog`** — фильтры+сортировка на клиенте
  (чистые функции в `utils/catalog/catalogClientFilter.ts`). Сортировка
  «новизне» идёт по `aired_on` (дата старта), затем `year`, затем id/updated_at;
  она исключает анонсы, если пользователь явно не выбрал статус «Анонс».
  «дате добавления» идёт от новых id к старым, «рейтингу» — по оценке.
- **`useCatalogFilters`** — фильтры в URL search-params (переживают reload);
  маппинги и подписи в `utils/catalog/catalogFilters.ts` и
  `utils/catalog/catalogData.ts`.
- **Плеер** (`features/animepage`): `useAnimePlayerState` — активная серия,
  ...
  озвучки, ссылка iframe = ссылка озвучки + `?episode=N` (`utils/anime/kodikLink.ts`).
  По клику на «Серия №» открывается поле ввода под строкой серий; число больше
  доступного выбирает последнюю доступную серию. В блоке кадров стрелки
  показываются только когда кадров больше пяти.
...
- **Аккаунты** (`services/auth.py`, `db/users.py`): регистрация/логин, scrypt-хеши
  паролей, сессии в коллекции `sessions` с TTL-индексом (30 дней; протухшие
  сессии Mongo удаляет сам). id юзера — `ObjectId` (на API сериализуется в
  строку). Токен хранится в localStorage, юзер
...
- **Комментарии/отзывы** (`routers/comments.py`, `db/comments.py`): список,
  создание, голоса (лайки), **древовидные ответы любой глубины** (`parent_id`;
  фронт `CommentThread` рекурсивно рендерит со сворачиванием ветки и «Ещё N
  ответов»), edit/delete. Удаление рекурсивно сносит всю подветку. Все мутации
  перепроверяют сессию и владельца на сервере (автор правит своё; админ удаляет
  любое, но не редактирует).
- **Админ-панель** (`/admin`, только для роли admin; `features/admin/`,
  `routers/admin_*`): управление пользователями (роль, бан, сброс пароля),
  CMS статических страниц, модерация комментариев (список + удаление,
  `routers/admin_comments.py`), журнал admin-действий (`admin_audit`). Доступ
  проверяется на сервере через `admin_auth`.
- **Статические страницы** (`routers/static_pages.py` + `admin_static_pages.py`,
  `db/static_pages.py`): тексты agreement/privacy/copyright хранятся в Mongo
  (коллекция `static_pages`, `_id` = slug), редактируются в админке, отдаются
  публично и рендерятся через `features/static-pages/StaticPageView` + `pagesApi`.
  БД — источник правды: дефолты из `db/static_page_content/*.txt` заливаются один
  раз (seed, `$setOnInsert`), дальше существующие документы не перетираются.
...
Сайт — SPA для пользователей, но backend отдаёт HTML с серверной SEO-вставкой
для роботов и соцскрейперов:

- **`routers/frontend_host.py`** зарегистрирован последним и отдаёт frontend
  build. Для HTML-запросов он вставляет per-route `<head>` и crawlable `<body>`
  из Mongo через `services/seo_render.py`. Неизвестные SPA-роуты получают
  настоящий `404` + `noindex`, а не soft-404.
...
- **JSON-LD** (`utils/animepage/structuredData.ts`): для тайтла `Movie`/`TVSeries` +
  `BreadcrumbList`.
- **`robots.txt`** (`routers/seo.py`): запрет `/admin`, `/profile`
...
- **Деплой**: production-сервер должен отдавать frontend build через backend
  или проксировать HTML-запросы на `routers/frontend_host.py`; иначе SEO-вставка
  и crawlable body не попадут к ботам. Задай `SITE_URL` для абсолютных URL.
...
```env
MONGODB_URI=mongodb://localhost:27017   # строка подключения (локально или Atlas)
MONGODB_DB=animewatch                    # имя базы
DATA_DIR=./data                          # локальные файлы (аватары)
LEGACY_SQLITE_PATH=./data/anime-viewer.db  # только для migrate_sqlite_to_mongo
